...
![image-20220614234125109](数据结构与算法_韩顺平.assets/image-20220614234125109.png)
...
![img](数据结构与算法_韩顺平.assets/o_200826025614图片2.png)

![img](数据结构与算法_韩顺平.assets/o_200826025624图片3.png)
...
1. 树有很多种， **每个节点最多只能有两个子节点**的一种形式称为**二叉树**。

2. 二叉树的子节点分为左节点和右节点

3. 示意图

   ![img](数据结构与算法_韩顺平.assets/o_200827115944微信截图_20200827195934.png)
...
1. 堆排序是利用堆这种数据结构而设计的一种排序算法， 堆排序是一种**选择排序**， 它的最坏， 最好， 平均时间复杂度均为 O(nlogn)， 它也是**不稳定排序**。

2. 堆是具有以下性质的**完全二叉树**： 每个结点的值都大于或等于其左右孩子结点的值， 称为**大顶堆**，注意 : 没有要求结点的左孩子的值和右孩子的值的大小关系。

3. 每个结点的值都小于或等于其左右孩子结点的值， 称为**小顶堆**

4. 大顶堆举例说明

   ![img](数据结构与算法_韩顺平.assets/o_200828111629图片9.png)

   我们对堆中的结点按层进行编号，映射到数组中就是下面这个样子:

   ![img](数据结构与算法_韩顺平.assets/o_200828111722图片10.png)
...
5. 小顶堆举例说明
   ...
   ![img](数据结构与算法_韩顺平.assets/o_200828112117图片11.png)
...
![img](数据结构与算法_韩顺平.assets/o_200912023132图片2.png)
...
2. 具有以下 **特点**： **它是一 棵空树或它的左右两个子树的高度差的绝对值不超过 1， 并且左右两个子树都是一棵平衡二叉树**。 平衡二叉树的常用实现方法有**红黑树、 AVL、 替罪羊树、 Treap、 伸展树**等
...
![img](数据结构与算法_韩顺平.assets/o_200913011209屏幕截图 2020-09-13 091156.png)
...
![img](数据结构与算法_韩顺平.assets/o_2009130156461122.png)
...
”部分匹配” 的实质是， 有时候， 字符串头部和尾部会有重复。 比如， ”ABCDAB” 之中有两个”AB” ， 那么它的”部分匹配值” 就是 2（”AB” 的长度） 。搜索词移动的时候， **第一个”AB” 向后移动 (6-2) 位**（字符串长度-部分匹配值） ， 就可以来到第二个”AB” 的位置。

![img](数据结构与算法_韩顺平.assets/o_2009130159392233.png)
...
![img](数据结构与算法_韩顺平.assets/o_200913023918111.jpg)
